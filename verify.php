...
<body>
  ...
  if(isset($_GET['email']) && !empty($_GET['email']) ){ // adicionar quando hash existir: AND isset($_GET['hash']) && !empty($_GET['hash']
    $email = $_GET['email'];
    //$hash = mysqli_escape_string($_GET['hash']); // adicionar quando hash existir
    $search = mysqli_query($conn,"SELECT email, ativo FROM cliente WHERE email='".$email."' AND ativo='0'"); // perceber se isto faz sentido: or die(mysql_error());
